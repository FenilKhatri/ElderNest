...
const SearchFilterBar = ({ 
  ...
  const hasActive = search.trim() || filters.some((f) => f.value && f.value !== "" && f.value !== "all");
  const activeCount = filters.filter((f) => f.value && f.value !== "" && f.value !== "all").length;

  return (
    <div className={`relative rounded-2xl border border-slate-200/80 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition-shadow duration-300 ${className}`}>
      <div className="flex flex-col lg:flex-row items-stretch gap-0">
        ...
        {/* Divider */}
        {filters.length > 0 && (<div className="hidden lg:block w-px bg-slate-200 dark:bg-slate-700/60 my-2" />)}
